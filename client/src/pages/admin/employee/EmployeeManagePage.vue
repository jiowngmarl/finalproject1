<template>
  <div class="employee-page-container">
    ...
    <div class="employee-list-panel">
      ...
      <!-- 액션 버튼 (관리자 권한만) -->
      <div v-if="canManageAdmin" class="action-buttons">
        <va-button
          preset="secondary"
          icon="delete_outline"
      ...
      <div class="table-container">
        <table class="employee-table">
          <thead>
            <tr>
              <!-- 체크박스는 관리자 권한이 있을 때만 -->
              <th v-if="canManageAdmin" width="40">
                <va-checkbox
                  v-model="selectAll"
          ...
          <tbody>
            ...
            >
              <!-- 체크박스는 관리자 권한이 있을 때만 -->
              <td v-if="canManageAdmin" @click.stop>
                <va-checkbox
                  v-model="selectedIds"
    ...
    <!-- 우측: 사원 등록/수정 (관리자 권한만) -->
    <div v-if="canManageAdmin" class="employee-detail-panel">
...
async function saveEmployee() {
  ...
  try {
    ...
    if (selectedEmployee.value) {
      // 수정
      await axios.put(
        `http://localhost:3000/employee/${selectedEmployee.value.employee_id}`,
        employeeData,
      );
...
async function deleteSelected() {
  ...
  try {
    ...
    const response = await axios.post(
      "http://localhost:3000/employee/delete-multiple",
      {
