...
export const useAuthStore = defineStore("auth", () => {
  ...
  const displayName = computed(() => {
    if (user.value) {
      return (
        user.value.employee_name || `사원 ${user.value.employee_id}` || "사용자"
      );
    }
    return "계정";
  });
  ...
  const saveAuthData = (userData: User, userToken: string) => {
    ...
    console.log(
      "부서 정보:",
      userData.department_name || userData.department_code,
    );
    console.log(
      "권한 정보:",
      userData.department_code
        ? DEPARTMENT_PERMISSIONS[userData.department_code] || "권한 없음"
        : "권한 없음",
    );
  };
  ...
  const loadAuthData = (): boolean => {
    try {
      ...
      if (authData?.user && authData?.token) {
        ...
        console.log(
          "저장된 인증 데이터 로드됨:",
          authData.user.employee_name || authData.user.employee_id,
        );
        console.log(
          "부서 정보:",
          authData.user.department_name || authData.user.department_code,
        );
        return true;
      }
    ...
    try {
      ...
      if (response.data.success) {
        ...
        console.log("권한 정보:", {
          ...
          permissions:
            userData.position === "관리자"
              ? "전체 권한"
              : userData.department_code
                ? DEPARTMENT_PERMISSIONS[userData.department_code] ||
                  "권한 없음"
                : "권한 없음",
        });
      ...
      if (error.response?.data?.message) {
        ...
        errorMessage =
          "로그인 요청 시간이 초과되었습니다. 네트워크를 확인해주세요.";
  ...
  const initialize = async () => {
    ...
    try {
      ...
      if (hasAuth && token.value) {
        console.log(
          "인증 데이터 있음, 사용자:",
          user.value?.employee_name || user.value?.employee_id,
        );
        console.log("부서 권한:", {
          department:
            user.value?.department_name || user.value?.department_code,
          position: user.value?.position,
          permissions:
            user.value?.position === "관리자"
              ? "전체 권한"
              : user.value?.department_code
                ? DEPARTMENT_PERMISSIONS[user.value.department_code] ||
                  "권한 없음"
                : "권한 없음",
        });
      } else {
